...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32, polygon_points=[]):
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
    ...
    def __next__(self):
        ...
        img1 = [pre_process_image(x, self.polygon_points) for x in img0]
        # Letterbox
        img = [letterbox(x, self.img_size, auto=self.rect, stride=self.stride)[0] for x in img1]
...
class LoadStreamsWithWork:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32, videoDatas=[]):
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
    ...
    def __next__(self):
        ...
        alarmConfig = []
        for i in range(0, len(self.videoDatas)):
            for item in self.videoDatas[i]['config']:
                alarmConfig.append(item['alarmConfigId'])
        img0 = self.imgs.copy()
        for i in range(0, len(self.sources)):
            source_temp.extend([self.sources[i]] * len(self.videoDatas[i]['config']))
            img0_temp.extend([img0[i]] * len(self.videoDatas[i]['config']))
            for item in self.videoDatas[i]['config']:
                img1.append(pre_process_image(img0[i], item['polygonPoints']))
